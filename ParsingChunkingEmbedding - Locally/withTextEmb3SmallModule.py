...
SLACK_TOKEN = "XXX"
CHANNEL_ID = "XXX"
VECTORDB_DIR = "text_emb_3_small_model"
OPENAI_API_KEY = "XXX"
TIME_WINDOW_MINUTES = 20  # grouping window

client = WebClient(token=SLACK_TOKEN)
openai_embed = OpenAIEmbeddings(model="text-embedding-3-small",openai_api_key=OPENAI_API_KEY)
...
def generate_tags_from_openai(text):
    prompt = f"""
    Analyze the following Slack conversation and generate a JSON with:
    - "intent" (1-2 words)
    - "keywords" (important terms in a list)
    ...
    {{
        ...
        "keywords": ["s3 public access", "security", "permissions"]
    }}
    """
    try:
...
def main():
    ...
    docs = build_documents(groups, channel_name)
